// src/app/dashboards/[role]/restaurants/list-load/page.tsx
'use client';
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const mapped: Job[] = arr.map((j: any) => ({
        ...
        extraServicesTotal: typeof j.extraServicesTotal === 'number' ? j.extraServicesTotal : undefined,
          ...
          <table className="min-w-full border-t border-neutral-200/70">
            <thead>
              <tr className="text-left text-sm text-neutral-500">
                ...
                <th className="px-6 py-3 font-medium w-[190px]"></th>
              </tr>
            </thead>
            <tbody>
              {loading && (
              ...
              {!loading && error && (
                <tr>
                  <td colSpan={9} className="px-6 py-8 whitespace-pre-wrap text-center text-rose-600">
                    {error}
                  </td>
                </tr>
              )}
              ...
              {!loading &&
                ...
                pageRows.map((r) => (
                  <tr key={r.id} className="border-t border-neutral-200/70 align-top hover:bg-neutral-50">
                    ...
                    <td className="px-3 py-4">
                      <div className="text-neutral-900">{fmtAppt(r.deliveryDate, r.deliveryTime)}</div>
                    </td>
                    <td className="px-3 py-4">
...
function EditJobModal({
  ...
  const [dDate, setDDate] = React.useState<string>(trToHtmlDate(row.deliveryDate || undefined));
          ...
          <div className="grid gap-4 sm:grid-cols-2 pb-28">
            ...
            <Field label="Ek Hizmetler Toplamı">
              <input
                ...
                onChange={(e) =>
                  setExtraServicesTotal(e.target.value === '' ? '' : Number(e.target.value))
                }
                className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 outline-none focus:ring-2 focus:ring-sky-200"
              />
            </Field>
